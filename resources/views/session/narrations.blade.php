...
@section('content')
<div class="mb-8"><a href="{{ route('session.readings', $student) }}" class="mb-4 inline-flex items-center gap-1 text-sm font-bold text-slate-500"><i class="bx bx-right-arrow-alt"></i> تغيير القراءة</a><p class="mb-2 text-sm font-bold text-gold">الخطوة الثالثة</p><h1 class="text-3xl font-bold text-emerald-950">اختر الرواية</h1><p class="mt-2 text-slate-500">{{ $qiraat->name }} · الطالب: <strong>{{ $student->name }}</strong></p></div>
<div class="grid gap-4 sm:grid-cols-2">@foreach($qiraat->narrations as $narration)@php
    $items=$attempts->get($narration->id,collect()); $active=$items->first(fn($a)=>!$a->completed_at); $completed=$items->whereNotNull('completed_at')->count();
